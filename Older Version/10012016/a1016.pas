...
procedure FileExist;
var
	infoFile, listFile, shortformFile: text;
	exist: boolean;
begin
	address := GetCurrentDir;
	repeat
		exist := false;
		if (FileExists(address+'\cardinfo.txt')) and (FileExists(address+'\cardlist.txt')) and (FileExists(address+'\cardshortform.txt')) then
			exist := true
		else
		begin
			//Generate cardinfo, cardlist, cardshortform text file.
		end;
	until exist;
end;
...
//	writeln(pointer);	writeln(outcome);	readln;
	...
	//						writeln('down');
		...
		writeln('            Exit');
  		...
  		writeln;
		writeln('Version Alpha 1.0.16                                                  By Goggle');
...
var
	...
	sf: string[3];
begin
	assign(cardsf, address+'\cardshortform.txt');
	reset(cardsf);
...
//					write(card[i,j]:3, ' ');
		...
		assign(cardinfo, address+'\cardinfo.txt');
		m := 0;
...
var
	...
	ch, ch2: char;
begin
	assign(cardlist, address+'\cardlist.txt');
	reset(cardlist);
