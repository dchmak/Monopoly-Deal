...
//	writeln(pointer);	writeln(outcome);	readln;
	...
	writeln('            Exit');
	writeln;
	writeln('Up/Down to move, Enter to confirm.');
	writeln('Version Alpha 1.0.20                                                  By Goggle');
...
procedure display(pNo: integer; var card: cardtype; var cardNo: cardNotype; var money: moneytype);
var
	i, j, k, m: integer;
begin
	for i := 1 to 4 do
...
//					write(card[i,j]:3, ' ');
					write(cardshortform[card[i,j]]:3, ' ');
				if j = 6 then
				begin
					case i of
						1:	gotoxy(1,7);
...
{		m := 0;
	...
	gotoxy(25,11);
	writeln('p for showing properties');
	gotoxy(20,13);
...
procedure showProperty(var properties: propertytype; var propmax: propmaxtype; var house, hotel: housetype);
var
	i, j, m: integer;
begin
	for i := 1 to 4 do
...
begin
	...
	begin
		...
		count2 := 0;	pointer := 0; accept := false;
...
//				writeln(accept);
				...
				if (haveNo[target] > 0) and (not accept) then
				begin
					gotoxy(1,30);
					write('You have ''Just Say NO'', do you want to use it? (Y/N)');
					repeat
						ch := readkey;
	...
	begin
		for k := 1 to 4 do
			...
			begin
                ...
                i := 0; count := 0; denied := false; accept := false;
...
//						writeln(accept);
						...
						if (haveNo[k] > 0) and (not accept) then
						begin
							gotoxy(1,30);
							write('You have ''Just Say NO'', do you want to use it? (Y/N)');
							repeat
...
procedure takeSet (user, target: byte; var properties: propertytype; var propmax: propmaxtype; var haveNo: haveNotype; var denied: boolean; var house, hotel: housetype);
var
	d,b,c,i, j, pointer: integer;
    ch: char;
	a: array[1..10] of integer;
	accept : boolean;
begin
	pointer := 0; denied := false; accept := false;
	repeat
		clrscr;
		...
		begin
			...
			begin
				...
				write(cardinfo[16+b,1], ': ', properties[d, b]);
				if properties[d, b] = propmax[b] then
					write(' (full)');
				if house[d, b] <> 0 then
					write('  ', house[d,b] div 3, ' house(s)');
				if hotel[d, b] <> 0 then
					write('  ', hotel[d,b] div 4, ' hotel(s)');
				c := c + 1;
			end;
		end;
		...
		begin
			...
			begin
				...
				case ch of
					...
					#32:	begin
								properties[target, a[pointer + 1]] := 0;
								properties[user, a[pointer + 1]] := propmax[pointer + 1];
							end;
				end;
			end;
		end;
...
procedure transProp (user, target, mode: byte; var properties: propertytype; var propmax: propmaxtype; var haveNo: haveNotype; var denied: boolean; var house, hotel: housetype); 					//mode: 1 = Forced Deal, 2 = Sly Deal;
var
	d,b,c,i, j, pointer: integer;
    ch: char;
	a: array[1..10] of integer;	
...
begin
	...
	repeat
		...
		j := 0;
		for i := 1 to 10 do
			if (properties[target, i] >= 1) and (properties[target, i] <> propmax[i]) then
			begin
				case i of
					1: write('Green     ');
					2: write('Dark Blue ');
	...
	begin
		...
		repeat
			...
			j := 0;
			for i := 1 to 10 do
				if (properties[user, i] >= 1) and (properties[target, i] <> propmax[i]) then
				begin
					case i of
						1: write('Green     ');
						2: write('Dark Blue ');
...
var
	...
	propmax: propmaxtype;
	money: moneytype;
	b,c,d,i, j, y, z, pNo, pointer, pointer2, target, code, action, propchoice, value, double: integer;
	input: string;
...
{		for i := 1 to 12 do
		...
		repeat
			repeat
				...
				If ord(ch)=0 then
					ch := readkey; 
			until ord(ch) in [75,77,13,105,112,32];
			...
			case ch of
				#75: 	begin									//left
							case pNo of
								...
								2:	if pointer < 6 then
										gotoxy(61 +	4 * pointer, 4)
									else
										gotoxy(61 +	4 * (pointer - 6), 6);
										
								3:	if pointer < 6 then
										gotoxy(61 + 4 * pointer, 20)
									else
										gotoxy(61 + 4 * (pointer - 6), 22);
				...
				#112:	begin									//p (property)
							clrscr;
							showProperty(properties,propmax,house,hotel);
						end;
				#32:	begin									//  (space)
							clrscr; leave := false;
							case card[pNo, pointer + 1] of
								1:	begin																				//Deal Breaker
										...
										repeat
											...
											readln(input);
											val(input, target, code);
											if (target < 1) or (target > 4) or (code = 1) then
												writeln('Invalid input, only 1 to 4 is valid');
										until target in [1..4];
										takeSet(pNo, target, properties, propmax, haveNo, denied,house,hotel);
										if denied then
										begin
											cardNo[target] := cardNo[target] - 1;
											for i := y to cardNo[target] do
								...
								3:	begin																				//Sly Deal
										...
										repeat
											...
											if (target < 1) or (target > 4) or (code = 1) then
												writeln('Invalid input, only 1 to 4 is valid');
										until target in [1..4];;
										transProp(pNo, target, 2, properties, propmax, haveNo, denied,house,hotel);
										if denied then
										begin
											cardNo[target] := cardNo[target] - 1;
											for i := y to cardNo[target] do
								...
								4:	begin																				//Forced Deal
										...
										repeat
											...
											if (target < 1) or (target > 4) or (code = 1) then
												writeln('Invalid input, only 1 to 4 is valid');
										until target in [1..4];
										transProp(pNo, target, 1, properties, propmax, haveNo, denied,house,hotel);
										if denied then
										begin
											cardNo[target] := cardNo[target] - 1;
											for i := y to cardNo[target] do
...
{			gotoxy(1,1);
		...
		begin
			gotoxy(30,1);
			writeln('Time for discard');
			gotoxy(27,2);
			writeln('(space) to remove a card');
			repeat
				ch := ReadKey;
				If ord(ch)=0 then
					ch:=readkey; 
...
begin
	writeln('Welcome to Monopoly Deal. Here I will teach you how to play this game.');
	delay(1000);
	writeln('When game starts you will have 7 cards in your hand, like this:');
	delay(1000);
	writeln;
	writeln('LRW  NO  SD RYM  TR  OP');
	writeln;
	writeln('$4');
	delay(3000);
	gotoxy(1,1);
	writeln('Please don''t fell scared to the characters, I will explain them later.  ');
	writeln('But first let me introduce how to select cards.                          ');
	gotoxy(1,8);
	writeln('The card that ''v'' is pointing is the card you are selecting.');
	writeln('Use Left/Right arrow key to move to card ''OP''.');
	pointer := 0;
	gotoxy(2,3);
	write('v');
	repeat
		repeat
			...
			If ord(ch)=0 then
				ch:=readkey; 
		until ord(ch) in [75,77];		
		
		case ch of
			#75: 	begin									//left
						if pointer < 6 then
							gotoxy(2 + 4 * pointer, 3)
						else
							gotoxy(2 + 4 * (pointer - 6), 5);
						write(' ');
						pointer := (pointer + 6) mod 7;
					end;
			#77:	begin									//right
						if pointer < 6 then
							gotoxy(2 + 4 * pointer, 3)
	...
	gotoxy(1,1);
	writeln('Good Job. Now you must be curious what does ''OP'' means.                  ');
	writeln('Let me teach you how to know what card it is.                              ');
	gotoxy(1,8);
	writeln('Press ''i'' to know more about the info.                        ');
	writeln('                                                                ');
	...
	writeln('Card Name: Orange properties');
	writeln;
	writeln('Cost: $2M');
	writeln;
	writeln('Detail info: No. of properties owned in set (Rent): 1($1M) 2($3M) (Full set)3($5M)');
	writeln;
	writeln('Here you can see all the information you need about this card.');
	delay(3000);
	writeln('Now we can press enter to return.');
	readln;
	
	clrscr;
	
	writeln('After knowing it''s a property card, maybe we should try to play it.');
	writeln;
	writeln('                     v');
	writeln('LRW  NO  SD RYM  TR  OP');
	writeln;
	writeln('$4');
	gotoxy(1,8);
	writeln('Press spacebar to use the card.');
	...
	gotoxy(1,1);
	writeln('Good, we have a orange property now.                               ');
	writeln;
	writeln;
	writeln('LRW  NO  SD RYM  TR  $4');
	writeln;
	writeln('         ');
	gotoxy(1,8);
	writeln('But you don''t see it appearear. Here''s why.');
	delay(1000);
	writeln('To conserve space, properties are shown in a differnet menu, just like the info');
	writeln('To enter that menu, press p.');
	repeat
		ch := readkey;
	until ord(ch) = 112;
	clrscr;
	...
	delay(3000);
	gotoxy(1,26);
	writeln('You win by having 3 full set of properties. Let me help you a bit');
	
	gotoxy(1,7);
	writeln('Orange Property: 2');
	writeln('Yellow Property: 3(full)');
	writeln('Red Property: 3(full)');
	gotoxy(1,28);
	writeln('Try to win now');
    win := false;
	readln;
	clrscr;
	writeln('Player 1                                                        ');
	...
	repeat
		...
		case ch of
			#75: 	begin									//left
						gotoxy(2 + 4 * pointer, 3);
						write(' ');
						pointer := (pointer + 5) mod 6;
					end;
			#77:	begin									//right
						gotoxy(2 + 4 * pointer, 3);
						write(' ');
						pointer := (pointer + 1) mod 6;
					end;
			#32:	begin
						if pointer = 2 then
						begin
	...
	writeln('This is the end of this trial game. If you want to try again,');
	writeln('you can go to tutorial section.');
	writeln('Also check out other tutorial for more detail.');
	writeln;	
	writeln('I hope you enjoy this game');
	delay(2000);
 	clrscr;    
	writeln('  Trial') ;
	writeln('  Control');
	...
	repeat
		...
		case i of
			1:	if input = 1 then
				begin
					newP := 1;
					writeln('It seems like you are new.');
					writeln('Would you like to have a trial game as a tutorial?(Y/N)');
					repeat
						ch := readkey;
					until ord(ch) in [121,110,89,78];
					clrscr;
					if ord(ch) in [121,89] then
						t_trial;
				end;
			2:	cur_textcolor := input;
		end;
	until eof(opt);
	close(opt);
	if newP = 1 then
